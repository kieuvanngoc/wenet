 ...
 */
	...
	/*
	 * Add function getSession
	 */
	function getSessionTable(){
		global $database;
		
		$query = "SELECT session_id FROM ".TBL_SESSION." WHERE session_id='".session_id()."' LIMIT 1";
		$result = $database->db_query($query);
		$object = $database->db_fetch_object($result);
		if (!$object) return false;
		else return $object->session_id;
	}
	
	/*
	 * Add function remove expired sessions from the database.
	 */
	function removeSession(){
		...
		if ($time % 2)
		{
			...
			$sql = "DELETE FROM ".TBL_SESSION." WHERE Cast(session_time as Char) < ".(int) ($time - $session->_Engine["none"]->settings["expire"]);
			$database->db_query($sql);
		}
	}
	
	/*
   	* Add function checkSession
   	*/
	function checkSession()
	{
		global $database, $user, $session;
		
		$query = "SELECT session_id FROM ".TBL_SESSION." WHERE session_id='".session_id()."'";
		$result = $database->db_query($query);
		$object = $database->db_fetch_object($result);
		...
		if (!$object)
		{
			if ( !$user->user_exists ){
				...
				$session_guest = 0;
				$session_user_id = $user->user_info["id"];
				$session_user_name = $user->user_info["ten_dang_nhap"];
				$session_user_type = $user->user_type;
			}
			// Insert table tbl_sessions
			$sql = "INSERT INTO ".TBL_SESSION." VALUES ('".session_id()."', ".$session_guest.", '".$session->sessionTime."', ".$session_user_id.", '".$session_user_name."', ".$session_user_type.")";
